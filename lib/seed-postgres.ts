...
export async function seedDatabase() {
  try {
    ...
    const existingUser = await sql`SELECT * FROM users WHERE email = 'iris.student@email.com'`;
    ...
    const userResult = await sql`
      ...
      VALUES ('iris.student@email.com', ${passwordHash}, 'Iris M.', 'Iris', 'IM', 'from-pink-500 to-rose-500', 12, 'ciencias', 18.0)
      RETURNING id
    `;
    ...
    for (const subject of subjects) {
      const result = await sql`
        ...
        VALUES (${userId}, ${subject.name}, ${subject.emoji}, ${subject.gradient}, ${subject.average}, ${subject.progress}, ${subject.next})
        RETURNING id
      `;
    ...
    for (const task of tasks) {
      await sql`
        ...
        VALUES (${userId}, ${subjectIds[task.subjectIdx]}, ${task.text}, ${today}, ${task.time}, ${task.done})
      `;
    }
    ...
    for (const exam of exams) {
      await sql`
        ...
        VALUES (${userId}, ${subjectIds[exam.subjectIdx]}, ${exam.title}, ${exam.type}, ${exam.year}, ${exam.phase}, ${exam.score}, ${exam.status}, 150, 20)
      `;
    }
    ...
    await sql`
      ...
      VALUES (${userId}, 12, 2450, 142, 1240, 12, 18)
    `;
    ...
    for (const badge of badges) {
      const result = await sql`
        ...
        VALUES (${badge.name}, ${badge.icon}, ${badge.color}, ${badge.type}, ${badge.value})
        RETURNING id
      `;
      badgeIds.push(result.rows[0].id);
    }
    ...
    for (const msg of chatMessages) {
      await sql`
        ...
        VALUES (${userId}, ${msg.role}, ${msg.content}, ${msg.subjectId})
      `;
    }
